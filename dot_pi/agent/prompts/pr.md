...
10. Write a truthful title and a short body.
11. The body must have exactly three simple points. Explain the change and name the Linear ticket.
12. Do not use em dashes, semicolons, or long comma-spliced sentences.
13. When attribution is on, add this line once near the end:

    `Generated with [Codex](https://openai.com/codex).`

    Put it before hidden data that must stay last. Do not add it when attribution is off.
14. Keep other author notes. Do not add the Codex line twice.
15. Save the title and body in separate temporary files.
16. Open both files with `$VISUAL`, then `$EDITOR`. Keep editor arguments such as `code --wait`.
17. Wait for the user to review them. Stop if either file is empty.
18. The title must be one line.
19. Check the template, hidden data, claims, and attribution. Ask before changing a user edit.
20. Show the title, body summary, base branch, head branch, draft state, and next command.
21. Use `gh pr edit` for an open pull request.
22. Use `gh pr create --draft` for a new one. Leave out `--draft` only when `--ready` was given.
23. Check the result with `gh pr view`. Report the URL, title, branches, draft state, and attribution count.
24. Report CI on its own. Do not merge.
